...
- A family whose faces come in more than one variant, such as a
  ...
  face.
- The Fonts group adds a font the design names nowhere. The picker is
  the Font control's, and a font added this way registers a face, so
  the author's CSS can set text in it by name. The row is set in that
  font, and the cross at its end takes it back out. A font the machine
  no longer has warns the way a missing design font warns.

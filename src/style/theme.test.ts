...
test("every design key has a default but a heading's font, a scene break's font and size, and each font's variant", () => {
  ...
  const optional = (key: string) =>
    /^heading-\d-font$/.test(key) || variant(key) || inherited(key);
